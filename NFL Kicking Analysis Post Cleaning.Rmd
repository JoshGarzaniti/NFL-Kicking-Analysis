...
```{r}
ggsave(here("nfl_kicking_current_kickers_avg_kicks_above_exp_plot.png"),
       current_kickers_avg_kicks_above_exp_plot)
```
...
final_ranked_current_kickers_gt = gt(final_ranked_current_kickers_dt)%>%
  tab_header(
    ...
    subtitle = "Generated from current season data")

final_ranked_current_kickers_gt
```

```{r}
gtsave(final_ranked_current_kickers_gt,
...
```{r}
final_ranked_current_kickers_100 = final_ranked_current_kickers%>%
  dplyr::select(kicker_player_name, Final_Rank, final_composite_score, 
                AMoE_current_rank, AMoE_FG_current_rank, long_current_rank, 
                clutch_current_rank, windy_current_rank, cold_current_rank, 
                epa_per_current_rank, wpa_per_current_rank)%>%
  left_join(ranked_kickers_total_current%>% 
              dplyr::select(kicker_player_name, kicks_attempted), 
            by = "kicker_player_name")%>%
  filter(kicks_attempted >= 100)%>%
  arrange(Final_Rank)%>%
  mutate(Final_Rank_100 = row_number())%>%
  dplyr::select(kicker_player_name, Final_Rank_100, Final_Rank, final_composite_score, 
                AMoE_current_rank, AMoE_FG_current_rank, long_current_rank, 
                clutch_current_rank, windy_current_rank, cold_current_rank, 
                epa_per_current_rank, wpa_per_current_rank)

head(final_ranked_current_kickers_100, 10)
