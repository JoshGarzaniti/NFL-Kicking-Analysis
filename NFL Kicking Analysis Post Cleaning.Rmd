...
## Analysis
```{r}
pbp_complete = read.csv(here("pbp_complete.csv"))
```
...
Creating my own theme for visuals:
```{r}
theme_nfl = theme_minimal(base_size = 14) +
  theme(
...
```{r}
distance_result = lm(result ~ kick_distance, data = pbp_complete_grouped)

summary(distance_result)
```
...
```{r}
ggsave(here("nfl_kicking_distance_vs_probability_plot.png"),
       distance_vs_probability_plot)
```
...
distance_vs_probability_plot_2 = ggplot(pbp_complete_binned, 
  ...
  annotate(
    "text", 
    x = max(pbp_complete_binned$kick_distance, na.rm = TRUE) - 5, 
    y = avg_prob + 0.05, 
    label = paste0("Average make probability = ", round(avg_prob, 3)),
    color = "red",
    size = 3,
    hjust = 1.25)+
  theme_nfl+
...
```{r}
ggsave(here("nfl_kicking_distance_vs_probability_plot2.png"),
       distance_vs_probability_plot_2)
```
...
distance_vs_probability_roof = ggplot(pbp_complete_binned, 
  aes(x = kick_distance, y = probability_advanced, color = roof))+
...
```{r}
ggsave(here("nfl_kicking_distance_vs_probability_roof.png"),
       distance_vs_probability_roof)
```
...
distance_vs_probability_surface = ggplot(pbp_complete_binned, 
  aes(x = kick_distance, y = probability_advanced, color = surface))+
  geom_point(aes(size = group_n), alpha = 0.6)+
  geom_smooth(se = FALSE)+
  geom_hline(
    yintercept = mean(pbp_complete$result, na.rm = TRUE), 
...
```{r}
ggsave(here("nfl_kicking_distance_vs_probability_surface.png"),
       distance_vs_probability_surface)
```
...
roof_prob_boxplot = ggplot(pbp_complete_binned, 
                             ...
                             geom_boxplot(fill = "navy", alpha = 0.7)+
                             theme_nfl+
...
```{r}
ggsave(here("nfl_kicking_roof_prob_boxplot.png"),
       roof_prob_boxplot)
```
...
long_distance_average_by_yard_plot = ggplot(long_distance_average_by_yard, 
                                            ...
                                            geom_point(aes(size = tries), color = 'navy')+
                                            geom_smooth()+
                                            theme_nfl+
...
```{r}
ggsave(here("nfl_kicking_long_distance_average_by_yard_plot.png"),
       long_distance_average_by_yard_plot)
```
...
```{r}
ggsave(here("nfl_kicking_distance_vs_probability_plot_3.png"),
       distance_vs_probability_plot_3)
```
...
Kicker_Random_Effects_plot1 = ggplot(random_effects_df, aes(x = reorder(kicker, `(Intercept)`), y = `(Intercept)`))+
  geom_point(aes(color = `(Intercept)`), size = 3)+  
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2)+
  coord_flip() +
  geom_hline(yintercept = 0, linetype = "dashed", color = 'red')+
  annotate(
    "text",
    x = length(unique(random_effects_df$kicker)) - 1,
    y = -1,
    label = "Log-odds baseline",
    color = "red",
    size = 4,
    hjust = 0)+
  ...
  dark_theme_minimal()+
  labs(
    title = "Current NFL Kicker-Specific Random Effects with 95% CI",
...
```{r}
ggsave(here("nfl_kicking_Kicker_Random_Effects_plot1.png"),
       Kicker_Random_Effects_plot1, height = 10, width = 8)
```
...
```{r}
ggsave(here("nfl_kicking_Kicker_Random_Effects_prob.png"),
       Kicker_Random_Effects_prob, height = 10, width = 8)
```
...
```{r}
ggsave(here("nfl_kicking_score_differential_make_plot.png"),
       score_differential_make_plot)
```
...
```{r}
write.csv(pbp_complete_grouped, 
          here("pbp_complete_grouped.csv"), 
          row.names = FALSE)
```
...
current_kickers_kicks_above_exp_plot = ggplot(ranked_kickers_total_current, 
  ...
  geom_point(aes(color = total_above_expected), size = 3)+
  ...
  scale_color_gradient2(
    ...
    mid = "yellow",   
    high = "green",   
    midpoint = mean_total_above_expected,     
    name = "Effect")+
...
```{r}
ggsave(here("nfl_kicking_current_kickers_kicks_above_exp_plot.png"),
       current_kickers_kicks_above_exp_plot)
```
...
current_kickers_avg_above_exp_plot = ggplot(ranked_kickers_average_current, 
  ...
  size = kicks_attempted))+
  geom_point(aes(color = average_above_expected), size = 3)+
  ...
  scale_color_gradient2(
    ...
    mid = "yellow",   
    high = "green",   
    midpoint = mean_avg_above_expected,     
    name = "Effect")+
  theme(axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
    panel.grid = element_blank())+
...
```{r}
ggsave(here("nfl_kicking_current_kickers_avg_above_exp_plot.png"),
       current_kickers_avg_above_exp_plot)
```
AKoE for Kickers with 100 Snaps
```{r}
ranked_kickers_average_current_100 = ranked_kickers%>%
  group_by(kicker_player_name)%>%
  summarise(kicks_attempted = n(),
            kicks_made = sum(result),
            percentage = (kicks_made / kicks_attempted),
            ...
            average_above_expected = mean(makes_above_expected),
            .groups = 'drop')%>%
  filter(kicker_player_name %in% c("J.Bates", "J.Tucker", "C.Boswell", "C.Dicker", "B.Aubrey", "C.McLaughlin", "J.Elliott", "T.Bass", "J.Sanders", "H.Butker", "T.Loop", "K.Fairbairn", "C.Santos", "J.Karty", "M.Gay", "W.Lutz", "E.McPherson", "D.Carlson", "B.McManus", "J.Myers", "W.Reichard", "Y.Koo", "Z.Gonzalez", "J.Moody", "C.Little", "A.Borregales", "C.Ryland", "G.Gano", "D.Hopkins", "M.Wright", "S.Shrader", "B.Grupe", "J.Slye", "J.McAtamney", "J.Parker Romo", "C.Davis", "A.Carlson", "G.Zuerlein", "N.Folk", "G.Joseph", "E.Pineiro", "A.Seibert", "M.Prater", "B.Narveson", "L.Havrisik", "C.York", "T.Brown", "C.Dunn", "R.Bullock", "B.Maher", "M.Ammendola", "N.Sciba", "J.Garibay", "R.Patterson", "J. McCourt", "C.Shudak", "M.Badgley"), kicks_attempted >= 100)%>%
  arrange(desc(average_above_expected))%>%
  mutate(AMoE_current_rank = row_number())

head(ranked_kickers_average_current_100, 10)
```

```{r}
mean_avg_above_expected_100 = mean(ranked_kickers_average_current_100$average_above_expected)

current_kickers_avg_above_exp_100_plot = ggplot(ranked_kickers_average_current_100, 
  mapping = aes(x = kicker_player_name,                                                     y = average_above_expected,
  size = kicks_attempted))+
  geom_point(aes(color = average_above_expected), size = 3)+
  geom_hline(yintercept = mean_avg_above_expected, 
  ...
  theme(axis.text.x = element_blank(),
    ...
    panel.grid = element_blank())+
  labs(title = "Average Kicks Made Above Expected", 
       subtitle = "Current NFL Kickers with at least 100 attempts",
       x = "",
       y = "Average KoE")

current_kickers_avg_above_exp_100_plot

```

```{r}
ggsave(here("nfl_kicking_current_kickers_avg_above_exp_100_plot.png"),
       current_kickers_avg_above_exp_100_plot)
```
...
current_kickers_fgs_above_exp_plot = ggplot(ranked_kickers_fg_current, 
  mapping = aes(x = kicker_player_name, 
  y = total_above_expected,
  size = fgs_attempted))+
  geom_point(aes(color = total_above_expected), size = 3)+
  ...
  scale_color_gradient2(
    ...
    mid = "yellow",   
    high = "green",   
    midpoint = mean_fg_above_expected,     
    name = "Effect")+
  theme(axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
    panel.grid = element_blank())+
...
current_kickers_fgs_above_exp_plot
```

```{r}
ggsave(here("nfl_kicking_current_kickers_fgs_above_exp_plot.png"),
       current_kickers_fgs_above_exp_plot, height = 5, width = 8)
```
...
current_kickers_avg_fgs_above_exp_plot = ggplot(ranked_kickers_fg_current_avg, 
  mapping = aes(x = kicker_player_name, 
  ...
  scale_color_gradient2(
    ...
    mid = "yellow",   
    high = "green",   
    midpoint = mean_avg_fg_above_expected,     
    name = "Effect")+
  theme(axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
    panel.grid = element_blank())+
...
current_kickers_avg_fgs_above_exp_plot
```

```{r}
ggsave(here("nfl_kicking_current_kickers_avg_fgs_above_exp_plot.png"),
       current_kickers_avg_fgs_above_exp_plot, height = 6, width = 8)
```

Average KoE (on FG Attempts) for kickers with 100 or more snaps
```{r}
kickers_100_attempts = ranked_kickers%>%
  group_by(kicker_player_name)%>%
  summarise(kicks_attempted = n(), .groups = "drop")%>%
  filter(kicks_attempted >= 100)%>%
  pull(kicker_player_name)


ranked_kickers_fg_current_avg_100 = ranked_kickers_fgs%>%
  filter(kicker_player_name %in% kickers_100_attempts)%>%
  group_by(kicker_player_name)%>%
  summarise(
    fgs_attempted = n(),
    fgs_made = sum(result),
...
head(ranked_kickers_fg_current_avg_100, 10)


```

```{r}
mean_avg_fg_above_expected_100 = mean(ranked_kickers_fg_current_avg_100$average_above_expected)


current_kickers_avg_fgs_above_exp_100_plot = ggplot(ranked_kickers_fg_current_avg_100, 
  mapping = aes(x = kicker_player_name, 
  y = average_above_expected,
  size = fgs_attempted))+
  geom_point(aes(color = average_above_expected), size = 3)+
  geom_hline(yintercept = mean_avg_fg_above_expected_100, 
             linetype = "dashed", color = "red")+
  theme_nfl+
  scale_color_gradient2(
    low = "red",     
  ...
  geom_text_repel(aes(label = kicker_player_name), 
                  ...
                  size = 3)+
  labs(title = "Average Field Goals Made Above Expected", 
       subtitle = "current NFL kickers with 100 attempts",
       x = "Player",
       y = "Average FGoE")

current_kickers_avg_fgs_above_exp_100_plot
```

```{r}
ggsave(here("nfl_kicking_current_kickers_avg_fgs_above_exp_100_plot.png"),
       current_kickers_avg_fgs_above_exp_100_plot, height = 6, width = 8)
```
...
current_kickers_kicks_above_exp_long_plot = ggplot(ranked_kickers_long_current, 
  ...
  scale_color_gradient2(
    ...
    mid = "yellow",   
    high = "green",   
    midpoint = mean_fg_above_expected_long,     
    name = "Effect")+
  theme(axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
    panel.grid = element_blank())+
...
current_kickers_avg_kicks_above_exp_long_plot = ggplot(ranked_kickers_long_current_avg, 
  ...
  scale_color_gradient2(
    ...
    mid = "yellow",   
    high = "green",   
    midpoint = mean_avg_fg_above_expected_long,     
    name = "Effect")+
  theme(axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
    panel.grid = element_blank())+
...
Historically, Justin Tucker has generated the most total epa, and ranks top 8 all time in epa per attempt (4th among those with 50+ attempts)
...
wpa_kickers = ranked_kickers%>%
  ...
  filter(kick_attempts >= 50) ##again you can manipulate this threshold
...
EPA_current_kickers_plot = ggplot(epa_kickers_current, 
  ...
  y = total_epa,
  size = kick_attempts))+
  geom_point(aes(color = total_epa), size = 3)+
...
EPA_per_current_kickers_plot = ggplot(epa_kickers_current, 
  ...
  y = epa_per_attempt,
  size = kick_attempts))+
  geom_point(aes(color = epa_per_attempt))+
  ...
  scale_color_gradient2(
    ...
    mid = "yellow",   
    high = "green",   
    midpoint = mean_avg_epa_per_attempt,     
    name = "Effect")+
  theme(axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
    panel.grid = element_blank())+
...
WPA_current_kickers_plot = ggplot(wpa_kickers_current, 
  ...
  y = total_wpa,
  size = kick_attempts))+
  geom_point(aes(color = total_wpa))+
...
WPA_per_current_kickers_plot = ggplot(wpa_kickers_current, 
  ...
  y = wpa_per_attempt,
  size = kick_attempts))+
  geom_point(aes(color = wpa_per_attempt))+
  ...
  scale_color_gradient2(
    ...
    mid = "yellow",   
    high = "green",   
    midpoint = mean_avg_wpa_per_attempt,     
    name = "Effect")+
  theme(axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
    panel.grid = element_blank())+
...
long_rank_mean   = mean(final_ranked_current_kickers$long_current_rank, na.rm = TRUE)
clutch_rank_mean = mean(final_ranked_current_kickers$clutch_current_rank, na.rm = TRUE)
windy_rank_mean  = mean(final_ranked_current_kickers$windy_current_rank, na.rm = TRUE)
cold_rank_mean   = mean(final_ranked_current_kickers$cold_current_rank, na.rm = TRUE)

final_ranked_current_kickers = final_ranked_current_kickers %>%
  mutate(long_current_rank   = ifelse(is.na(long_current_rank), long_rank_mean, long_current_rank),
         clutch_current_rank = ifelse(is.na(clutch_current_rank), clutch_rank_mean, clutch_current_rank),
         windy_current_rank  = ifelse(is.na(windy_current_rank), windy_rank_mean, windy_current_rank),
         cold_current_rank   = ifelse(is.na(cold_current_rank), cold_rank_mean, cold_current_rank)) %>%
...
final_ranked_current_kickers_dt = as.data.table(final_ranked_current_kickers)
```


```{r}
final_ranked_current_kickers_gt = gt(final_ranked_current_kickers_dt)%>%
  tab_header(
    title = "Final Ranked Current Kickers",
    subtitle = "As of the 2025 preseason")

final_ranked_current_kickers_gt
```

```{r}
gtsave(
  final_ranked_current_kickers_gt,
  here("final_ranked_current_kickers.png"),
  vwidth = 2000,
  vheight = 800)
```

##What would the ranking be if say we only wanted to look at current kickers with at least 100 attempts:
Using:

ranked_kickers_average_current_100

ranked_kickers_fg_current_avg_100

Long current 100
```{r}
ranked_kickers_long_current_100 = ranked_kickers_long%>%
  filter(kicker_player_name %in% kickers_100_attempts)%>%
  group_by(kicker_player_name)%>%
  summarise(fgs_attempted = n(),
            fgs_made = sum(result),
  ...
  filter(kicker_player_name %in% c("J.Bates", "J.Tucker", "C.Boswell", "C.Dicker", "B.Aubrey", "C.McLaughlin", "J.Elliott", "T.Bass", "J.Sanders", "H.Butker", "T.Loop", "K.Fairbairn", "C.Santos", "J.Karty", "M.Gay", "W.Lutz", "E.McPherson", "D.Carlson", "B.McManus", "J.Myers", "W.Reichard", "Y.Koo", "Z.Gonzalez", "J.Moody", "C.Little", "A.Borregales", "C.Ryland", "G.Gano", "D.Hopkins", "M.Wright", "S.Shrader", "B.Grupe", "J.Slye", "J.McAtamney", "J.Parker Romo", "C.Davis", "A.Carlson", "G.Zuerlein", "N.Folk", "G.Joseph", "E.Pineiro", "A.Seibert", "M.Prater", "B.Narveson", "L.Havrisik", "C.York", "T.Brown", "C.Dunn", "R.Bullock", "B.Maher", "M.Ammendola", "N.Sciba", "J.Garibay", "R.Patterson", "J. McCourt", "C.Shudak", "M.Badgley"))%>%
  arrange(desc(total_above_expected))%>%
 mutate(Long_current_100_rank = row_number())

head(ranked_kickers_long_current_100, 10)
```

clutch current 100
```{r}
clutch_kicker_current_100 = ranked_kickers_clutch%>%
  filter(is_clutch == 1, kicker_player_name %in% kickers_100_attempts)%>%
  group_by(kicker_player_name)%>%
  summarise(
    clutch_attempts = n(),
    clutch_makes = sum(result),
  ...
  filter(kicker_player_name %in% c("J.Bates", "J.Tucker", "C.Boswell", "C.Dicker", "B.Aubrey", "C.McLaughlin", "J.Elliott", "T.Bass", "J.Sanders", "H.Butker", "T.Loop", "K.Fairbairn", "C.Santos", "J.Karty", "M.Gay", "W.Lutz", "E.McPherson", "D.Carlson", "B.McManus", "J.Myers", "W.Reichard", "Y.Koo", "Z.Gonzalez", "J.Moody", "C.Little", "A.Borregales", "C.Ryland", "G.Gano", "D.Hopkins", "M.Wright", "S.Shrader", "B.Grupe", "J.Slye", "J.McAtamney", "J.Parker Romo", "C.Davis", "A.Carlson", "G.Zuerlein", "N.Folk", "G.Joseph", "E.Pineiro", "A.Seibert", "M.Prater", "B.Narveson", "L.Havrisik", "C.York", "T.Brown", "C.Dunn", "R.Bullock", "B.Maher", "M.Ammendola", "N.Sciba", "J.Garibay", "R.Patterson", "J. McCourt", "C.Shudak", "M.Badgley"))%>%
  arrange(desc(clutch_avg_above_exp))%>%
  mutate(clutch_current_100_rank = row_number())

head(clutch_kicker_current_100, 10)
```

cold current 100
```{r}
cold_kicks_current_100 = ranked_kickers%>%
  filter(
    temp <= 40,
    kicker_player_name %in% kickers_100_attempts)%>%
  group_by(kicker_player_name)%>%
  summarise(
    cold_attempts = n(),
    cold_makes = sum(result),
  ...
  filter(kicker_player_name %in% c("J.Bates", "J.Tucker", "C.Boswell", "C.Dicker", "B.Aubrey", "C.McLaughlin", "J.Elliott", "T.Bass", "J.Sanders", "H.Butker", "T.Loop", "K.Fairbairn", "C.Santos", "J.Karty", "M.Gay", "W.Lutz", "E.McPherson", "D.Carlson", "B.McManus", "J.Myers", "W.Reichard", "Y.Koo", "Z.Gonzalez", "J.Moody", "C.Little", "A.Borregales", "C.Ryland", "G.Gano", "D.Hopkins", "M.Wright", "S.Shrader", "B.Grupe", "J.Slye", "J.McAtamney", "J.Parker Romo", "C.Davis", "A.Carlson", "G.Zuerlein", "N.Folk", "G.Joseph", "E.Pineiro", "A.Seibert", "M.Prater", "B.Narveson", "L.Havrisik", "C.York", "T.Brown", "C.Dunn", "R.Bullock", "B.Maher", "M.Ammendola", "N.Sciba", "J.Garibay", "R.Patterson", "J. McCourt", "C.Shudak", "M.Badgley"))%>%
  arrange(desc(cold_avg_above_exp))%>%
  mutate(cold_current_100_rank = row_number())

head(cold_kicks_current_100, 10)
```

windy current 100
```{r}
windy_kicks_current_100 = ranked_kickers%>%
  filter(
    wind >= 10,
    kicker_player_name %in% kickers_100_attempts)%>%
  group_by(kicker_player_name)%>%
  ...
  filter(kicker_player_name %in% c("J.Bates", "J.Tucker", "C.Boswell", "C.Dicker", "B.Aubrey", "C.McLaughlin", "J.Elliott", "T.Bass", "J.Sanders", "H.Butker", "T.Loop", "K.Fairbairn", "C.Santos", "J.Karty", "M.Gay", "W.Lutz", "E.McPherson", "D.Carlson", "B.McManus", "J.Myers", "W.Reichard", "Y.Koo", "Z.Gonzalez", "J.Moody", "C.Little", "A.Borregales", "C.Ryland", "G.Gano", "D.Hopkins", "M.Wright", "S.Shrader", "B.Grupe", "J.Slye", "J.McAtamney", "J.Parker Romo", "C.Davis", "A.Carlson", "G.Zuerlein", "N.Folk", "G.Joseph", "E.Pineiro", "A.Seibert", "M.Prater", "B.Narveson", "L.Havrisik", "C.York", "T.Brown", "C.Dunn", "R.Bullock", "B.Maher", "M.Ammendola", "N.Sciba", "J.Garibay", "R.Patterson", "J. McCourt", "C.Shudak", "M.Badgley"))%>%
  arrange(desc(windy_avg_above_exp))%>%
  mutate(windy_current_100_rank = row_number())

head(windy_kicks_current_100, 10)
```

EPA Current 100
```{r}
epa_kickers_current_100 = ranked_kickers%>%
  group_by(kicker_player_name)%>%
  summarise(
    kick_attempts = n(),
    total_epa = sum(epa),
  ...
  filter(
    kicker_player_name %in% kickers_100_attempts)%>%
  arrange(desc(epa_per_attempt))%>%
  mutate(epa_per_current_100_rank = row_number())

head(epa_kickers_current_100, 10)
```

WPA Current 100
```{r}
wpa_kickers_current_100 = ranked_kickers%>%
  group_by(kicker_player_name)%>%
  summarise(
    kick_attempts = n(),
    total_wpa = sum(wpa),
  ...
  filter(
    kicker_player_name %in% kickers_100_attempts)%>%
  arrange(desc(wpa_per_attempt))%>%
  mutate(wpa_per_current_100_rank = row_number())

head(wpa_kickers_current_100, 10)
```


```{r}
final_ranked_current_kickers_100 = ranked_kickers_average_current_100%>%
  dplyr::select(kicker_player_name, AMoE_current_rank)%>%
  left_join(ranked_kickers_fg_current_avg_100%>%
              dplyr::select(kicker_player_name, AMoE_FG_current_rank),
            by = "kicker_player_name")%>%
  left_join(ranked_kickers_long_current_100%>%
              dplyr::select(kicker_player_name, Long_current_100_rank),
            by = "kicker_player_name")%>%
  left_join(clutch_kicker_current_100%>%
              dplyr::select(kicker_player_name, clutch_current_100_rank),
            by = "kicker_player_name")%>%
  left_join(cold_kicks_current_100%>%
              dplyr::select(kicker_player_name, cold_current_100_rank),
            by = "kicker_player_name")%>%
  left_join(windy_kicks_current_100%>%
              dplyr::select(kicker_player_name, windy_current_100_rank),
            by = "kicker_player_name")%>%
  left_join(epa_kickers_current_100%>%
              dplyr::select(kicker_player_name, epa_per_current_100_rank),
            by = "kicker_player_name")%>%
  left_join(wpa_kickers_current_100%>%
              dplyr::select(kicker_player_name, wpa_per_current_100_rank),
            by = "kicker_player_name")%>%
  left_join(final_ranked_current_kickers%>%
              dplyr::select(kicker_player_name, Final_Rank),
            by = "kicker_player_name")


long_rank_mean = mean(ranked_kickers_long_current_100$Long_current_100_rank, na.rm = TRUE)
clutch_rank_mean = mean(clutch_kicker_current_100$clutch_current_100_rank, na.rm = TRUE)
windy_rank_mean  = mean(windy_kicks_current_100$windy_current_100_rank, na.rm = TRUE)
cold_rank_mean   = mean(cold_kicks_current_100$cold_current_100_rank, na.rm = TRUE)


final_ranked_current_kickers_100 = final_ranked_current_kickers_100%>%
  mutate(
    Long_current_100_rank   = ifelse(is.na(Long_current_100_rank),   long_rank_mean,   Long_current_100_rank),
    clutch_current_100_rank = ifelse(is.na(clutch_current_100_rank), clutch_rank_mean, clutch_current_100_rank),
    windy_current_100_rank  = ifelse(is.na(windy_current_100_rank),  windy_rank_mean,  windy_current_100_rank),
    cold_current_100_rank   = ifelse(is.na(cold_current_100_rank),   cold_rank_mean,   cold_current_100_rank))%>%
  mutate(final_composite_score_100 = (
    AMoE_current_rank +
    AMoE_FG_current_rank +
    Long_current_100_rank +
    clutch_current_100_rank +
    windy_current_100_rank +
    cold_current_100_rank +
    epa_per_current_100_rank +
    wpa_per_current_100_rank) / 8)%>%
  arrange(final_composite_score_100)%>%
  mutate(Final_Rank_100 = row_number())%>%
  dplyr::select(
    kicker_player_name,
    Final_Rank_100,
    Final_Rank,
    final_composite_score_100,
    AMoE_current_rank,
    AMoE_FG_current_rank,
    Long_current_100_rank,
    clutch_current_100_rank,
    ...
    cold_current_100_rank,
    epa_per_current_100_rank,
    wpa_per_current_100_rank)


final_ranked_current_kickers_100_dt = as.data.table(final_ranked_current_kickers_100)
```


```{r}
final_ranked_current_kickers_100_gt = gt(final_ranked_current_kickers_100_dt)%>%
  tab_header(
    title = "Final Ranked Current Kickers",
    subtitle = "with at least 100 kicking attempts")

final_ranked_current_kickers_100_gt
```

```{r}
gtsave(final_ranked_current_kickers_100_gt,
       here("final_ranked_current_kickers_100.png"),
       vwidth = 2000,
  vheight = 800)
```
```{r}
final_ranked_current_kickers_100 = final_ranked_current_kickers_100%>%
  mutate(movement = Final_Rank- Final_Rank_100)%>%
  arrange(desc(movement))

head(final_ranked_current_kickers_100, 15)
```
...
Little, Shrader, and Karty exit the top 10 due to not having the volume of kick attempts


##Future work

Exploring simple linear fits
...
Running a Bayesian mixed effect model to compute kicker intercepts

This way we are scaling effectively towards the mean (in terms of log effects on probability) for kickers that have minimal kick attempts (statistical outliers)

```{r}
bayesian_kicker_mixed_model = brm(formula = result ~ kick_distance_scaled + temp_scaled + roof + wind_scaled + surface + (1 | kicker_player_name), 
    data = pbp_complete,
...
summary(bayesian_kicker_mixed_model)
```

pulling the player estimate intervals 
```{r}
kicker_effects_brm = ranef(bayesian_kicker_mixed_model)$kicker_player_name

head(kicker_effects_brm, 10)
```

Taking the coefficients of those estimates
```{r}
kicker_draws = coef(bayesian_kicker_mixed_model)$kicker_player_name

head(kicker_draws, 10)
```

Mutating those estimates to probabilities
```{r}
bayesian_kicker_summary = kicker_effects_brm[,,"Intercept"]%>% 
  as.data.frame()%>%
  tibble::rownames_to_column("kicker")%>%
  mutate(probability = plogis(Estimate),
         lower = plogis(Q2.5),
         upper = plogis(Q97.5))%>%
  arrange(desc(probability))%>%
  filter(kicker %in% c("J.Bates", "J.Tucker", "C.Boswell", "C.Dicker", "B.Aubrey", "C.McLaughlin", "J.Elliott", "T.Bass", "J.Sanders", "H.Butker", "T.Loop", "K.Fairbairn", "C.Santos", "J.Karty", "M.Gay", "W.Lutz", "E.McPherson", "D.Carlson", "B.McManus", "J.Myers", "W.Reichard", "Y.Koo", "Z.Gonzalez", "J.Moody", "C.Little", "A.Borregales", "C.Ryland", "G.Gano", "D.Hopkins", "M.Wright", "S.Shrader", "B.Grupe", "J.Slye", "J.McAtamney", "J.Parker Romo", "C.Davis", "A.Carlson", "G.Zuerlein", "N.Folk", "G.Joseph", "E.Pineiro", "A.Seibert", "M.Prater", "B.Narveson", "L.Havrisik", "C.York", "T.Brown", "C.Dunn", "R.Bullock", "B.Maher", "M.Ammendola", "N.Sciba", "J.Garibay", "R.Patterson", "J. McCourt", "C.Shudak", "M.Badgley"))

head(bayesian_kicker_summary, 10)
```

Plotting kicker probabilities
```{r}
bayes_kicker_probs_plot = ggplot(bayesian_kicker_summary, aes(x = reorder(kicker, probability), y = probability))+
  geom_point(aes(color = probability), size = 3)+
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2)+
  coord_flip()+
  dark_theme_minimal()+
  scale_color_gradient2(low = "red", mid = "yellow", high = "green",
                        midpoint = mean(bayesian_kicker_summary$probability),
                        name = "Effect")+
  labs(title = "NFL Kicker-Specific Intercepts (brms)",
       subtitle = "Posterior means with 95% credible intervals",
       y = "Probability of Making a Kick (Intercept only)",
       x = "Kicker")

bayes_kicker_probs_plot
```



Given the model and the fixed variables we accounted for, there is a 95% probability that Tucker’s true baseline make probability under average conditions lies between ~69% and ~79%.
