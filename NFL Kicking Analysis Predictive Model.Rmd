...
abline(a = 0, b = 1, lty = 2, col = "gray")
```
Final Model SHAP Graph
```{r}
shap_values = shap.values(
  xgb_model = xgb_balanced_model_final,
  X_train   = balanced_train_data)
```
...
```
Saving SHAP graphr results
```{r}
png(filename = here("shap_summary_plot.png"),
...
save our xgb model
```{r}
xgb.save(xgb_balanced_model_final, here("nfl_kicking_balanced_model_final.model"))
```